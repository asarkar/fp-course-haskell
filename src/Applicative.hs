...
instance Applicative ExactlyOne where
  ...
  (<*>) = EO.mapExactlyOne . EO.runExactlyOne
...
instance Applicative ((->) t) where
  ...
  (<*>) :: (->) t (a -> b) -> (->) t a -> (->) t b
  -- f is a function that takes a type 't' and returns a function (a -> b)
  -- g is a function that takes a type 't' and returns a type 'a'
  (<*>) f g t = f t (g t)
...
-- sequence (x :. xs) = (:.) F.<$> x <*> sequence xs
sequence = L.foldRight (lift2 (:.)) (pure Nil)

-- | Replicate an effect a given number of times.
--
-- /Tip:/ Use `List#replicate`.
--
...
replicateA = (sequence .) . L.replicate
...
filtering p = L.foldRight (\a -> lift2 (\b -> if b then (a :.) else id) (p a)) (pure Nil)
